        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                ...
                <?php foreach (($items ?? []) as $user): ?>
                    <?php $targetIsProtected = (int) ($user['is_super_admin'] ?? 0) === 1 && empty($currentUserIsSuperAdmin); ?>
                    <tr>
                        <td><?= (int) $user['id'] ?></td>
                        <td><?= htmlspecialchars($user['first_name'] . ' ' . $user['last_name'], ENT_QUOTES, 'UTF-8') ?></td>
                        <td><?= htmlspecialchars($user['email'], ENT_QUOTES, 'UTF-8') ?></td>
                        <td>
                            <?php if (!empty($canAssignRoles) && !$targetIsProtected): ?>
                            ...
                            <?php else: ?>
                                <?= htmlspecialchars((string) ($user['role_name'] ?? $user['role']), ENT_QUOTES, 'UTF-8') ?>
                            <?php endif; ?>
                        </td>
                        <td>
                            <?php if (!empty($canEditUsers) && !$targetIsProtected): ?>
                        ...
                        <td class="d-flex gap-2 flex-wrap">
                            ...
                            <?php endif; ?>
                            <?php if ($targetIsProtected || (empty($canEditUsers) && empty($canDeleteUsers))): ?>
